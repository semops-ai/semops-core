...
CREATE TABLE IF NOT EXISTS entity (
  ...
  metadata JSONB NOT NULL DEFAULT '{}',             -- content_metadata_v1 / capability_metadata_v1 / repository_metadata_v1
  embedding vector(1536),                           -- OpenAI text-embedding-3-small for semantic search
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
-- Pattern coverage: documentation (FK), capability (implements edges), delivery (delivered_by chain)
-- DROP required: v8.0.0 changed column list (removed total_entity_count, added edge-based counts)
...
SELECT
  ...
  p.preferred_label,
  p.provenance,
  -- Documentation coverage: content entities linked via primary_pattern_id FK
...
COMMENT ON VIEW pattern_coverage IS 'Pattern coverage: documentation (FK), capability (implements edges), delivery (delivered_by via capability chain)';
