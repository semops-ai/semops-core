...
# ---------------------------------------------------------------------------
REPOS_YAML = Path(os.environ.get("DX_HUB_PATH", "../semops-dx-orchestrator")) / "docs" / "REPOS.yaml"
...
def _parse_capability_table(text: str, domain_classification: str) -> list[dict]:
    ...
    capabilities = []
    # Match rows: | `id` | Name | `pat1`, `pat2` | repo1, repo2 |
    row_re = re.compile(
    ...
    for match in row_re.finditer(text):
        ...
        patterns_raw = match.group(3).strip()
        repos_raw = match.group(4).strip()
...
def derive_lifecycle_stages(
    ...
    """Derive lifecycle_stage for capabilities and repos from edge coverage.

    Per ADR-0011 governance model:
    - Capability: 'active' if pattern_count > 0 AND repo_count > 0, 'draft' otherwise
    - Repository: 'active' if it delivers at least one capability, 'draft' otherwise
    ...
    """
    # Build lookup: capability_id -> {has_patterns, has_repos}
    cap_patterns: dict[str, int] = {}
    cap_repos: dict[str, int] = {}
    for edge in implements_edges:
        cap_patterns[edge["src_id"]] = cap_patterns.get(edge["src_id"], 0) + 1
    for edge in delivered_by_edges:
        cap_repos[edge["src_id"]] = cap_repos.get(edge["src_id"], 0) + 1

    for cap in capabilities:
        has_patterns = cap_patterns.get(cap["id"], 0) > 0
        has_repos = cap_repos.get(cap["id"], 0) > 0
        cap["metadata"]["lifecycle_stage"] = "active" if (has_patterns and has_repos) else "draft"

    # Build lookup: repo_id -> delivers any capabilities?
    repo_caps: dict[str, int] = {}
    for edge in delivered_by_edges:
        repo_caps[edge["dst_id"]] = repo_caps.get(edge["dst_id"], 0) + 1

    for repo in repos:
        has_caps = repo_caps.get(repo["id"], 0) > 0
        repo["metadata"]["lifecycle_stage"] = "active" if has_caps else "draft"
...
def main() -> int:
    ...
    active_caps = sum(1 for c in capabilities if c["metadata"].get("lifecycle_stage") == "active")
    draft_caps = len(capabilities) - active_caps
    active_repos = sum(1 for r in repos if r["metadata"].get("lifecycle_stage") == "active")
    draft_repos = len(repos) - active_repos
    console.print(
        f"Lifecycle: {active_caps} active / {draft_caps} draft capabilities, "
        f"{active_repos} active / {draft_repos} draft repos"
    )
